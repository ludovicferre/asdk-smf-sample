				...
				if (File.Exists(path)) {
					using (StreamReader sr = new StreamReader(path)) {
						while (sr.Peek() >= 0) {
							...
							create_software_product(in_data[1].Replace("\"", ""), "CWoC created product", in_data[2].Replace("\"", ""), in_data[3].Replace("\"", ""), in_data[8].Replace("\"", ""), in_data[4].Replace("\"", ""));
							// Console.WriteLine("{0} {1} {2} {3} {4} {5}", in_data[1].Replace("\"", ""), "CWoC created product", in_data[2], in_data[3], in_data[8], in_data[4]);
						}
					}
				return 0;
				} else {
...
/*	SKIP ALL PRODUCT GENERATION FOR TESTING PURPOSES

			#region Main work take place here #TODO: Export this to a function?
			...
			SoftwareProductDetails productDetails = managementLib.CreateSoftwareProduct(conf.product_name, conf.product_description, conf.company_name);
            ...
            string sql_mark_component_managed = String.Format("update Inv_Software_Component_State set IsManaged = 1 where _ResourceGuid = '{0}'", conf.software_component_guid);
            DatabaseAPI.ExecuteNonQuery(sql_mark_component_managed);


			// Associate Software Component if needed
			if (conf.software_component_guid != "") {
				string sql_associate_component = String.Format(sql.associate_component, conf.software_component_guid, productDetails.Guid.ToString());
				// Console.WriteLine("\n" + sql_associate_component + "\n");
				DatabaseAPI.ExecuteNonQuery(sql_associate_component);
			} else {
				// Console.WriteLine("SOWFTWARE COMPONENT GUID IS MISSING");
			}
			
			if (conf.product_version != "") {
				string sql_insert_version = String.Format(sql.ins_product_version, productDetails.Guid, conf.product_version);
			...
			if (conf.filter_product != "" && conf.filter_company != "") {
				
				string prod_filter = GetProductNameFilter(conf.filter_product);
				string sql_filter = String.Format(sql.product_filter_query, conf.filter_product, conf.filter_company, conf.filter_version);
				
				string sql_insert_filter = String.Format(sql.ins_product_filter, productDetails.Guid, conf.filter_product, conf.filter_company, conf.filter_version, sql_filter);
